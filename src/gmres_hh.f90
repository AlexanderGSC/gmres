MODULE GMRES_HH_MOD
    ...
    private
    public :: gmres_hh
    public :: gmres_hh_restarted
    integer:: stages = 50
CONTAINS
    subroutine gmres_hh_restarted(A,b,x,m,tol,final_err,v_err,n_out,stages_out)
        real(8), intent(in) :: A(:,:)   !Original matrix
        real(8), intent(in) :: b(:)     !Initial vector
        real(8), allocatable, intent(out):: x(:) !Sol. vector
        integer, intent(inout) :: m   !Max number of stages
        ...
        do k=1,stages
            ...
            if (h_val < 1d-16 .or. final_err(n_out) < tol) then
                stages_out = k
                exit
            end if
        end do  !restart loop
        call calculate_verr(P,w,y,v_err,n_out)
    end subroutine gmres_hh_restarted
