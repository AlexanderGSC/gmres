PROGRAM TEST_HILBERT
    ...
    if (n_args < 1) then
        print *, "usage ./test_hilbert <size> <max_iterations>"
        stop
    end if
    ...
    write(*, '(60("-"))') 
    call test_Hilbert_MGSR(nsize,max_iter)
    write(*, '(60("-"))')
    call test_Hilbert_HH(nsize,max_iter)
    write(*, '(60("-"))')
CONTAINS 
    SUBROUTINE test_hilbert_HH(nsize, max_iter)
        IMPLICIT none
        ...
        write(*,'(A)') 'GMRES Hilbert Matrix Test (Householder version)'
    ...
    END SUBROUTINE test_hilbert_HH
    SUBROUTINE test_hilbert_MGSR(nsize, max_iter)
        integer, intent(in)::nsize
        integer, intent(in)::max_iter
        real(8), allocatable::A(:,:),b(:),x(:),errn(:),verr(:)
        real(8)::tol
